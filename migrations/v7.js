...
describe('adapt-contrib-accordion - v7.3.0 > v7.7.0', async () => {
  let accordions;

  whereFromPlugin('adapt-contrib-accordion - from v7.3.0', { name: 'adapt-contrib-accordion', version: '<=7.7.0' });
  ...
  checkContent('adapt-contrib-accordion - check accordion._isCenterAligned atrribute', async () => {
  ...
  checkContent('adapt-contrib-accordion - check accordion._items._titleIcon atrribute', async () => {
    const isValid = accordions.every(accordion =>
      accordion._items.every(item =>
